    ...
    setBasket((prev) =>
      prev.map((item) => {
        if (item.product._id === productId) {
          const max = item.product.warehouseQuantity;
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-white border rounded-sm overflow-x-auto shadow-sm">
        {[1, 2, 3, 4].map((s) => (
          <div key={s} className="flex items-center flex-shrink-0">
            ...
            <span
              className={`ml-2 text-[10px] font-black uppercase tracking-widest ${step === s ? "text-primary" : "text-slate-400"}`}
            >
              {s === 1 && "Sotuvchi"}
              {s === 2 && "Tanlash"}
              {s === 3 && "Miqdor"}
            ...
            {s < 4 && (
              <ChevronRight size={14} className="mx-4 text-slate-300" />
            )}
          </div>
        ))}
      </div>
            ...
            {productsLoading
              ...
              : products.map((product) => {
                  ...
                  return (
                    <Card
                      key={product._id}
                      className={`rounded-sm border-slate-200 transition-shadow hover:shadow-md ${isInBasket ? "bg-slate-50/50 border-primary/30" : ""}`}
                    >
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-bold text-slate-400 uppercase">
                        ...
                        <span className="text-primary">
                          {item.product.warehouseQuantity}
                        </span>
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-3 self-end md:self-center">
                    <div className="flex items-center border border-slate-200 rounded-sm overflow-hidden h-9">
                      ...
                      <Button
                        ...
                        disabled={
                          item.quantity >= item.product.warehouseQuantity
                        }
                      >
                        <Plus size={14} />
                      </Button>
        ...
        <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-300">
          <Card className="rounded-sm border-primary/20 bg-primary/[0.02] shadow-none">
            <CardHeader className="pb-4 border-b border-primary/10">
              <CardTitle className="text-sm font-black uppercase text-primary tracking-widest text-center">
            ...
            <CardContent className="space-y-6 pt-6">
              ...
              <div className="flex justify-between items-center p-4 bg-slate-800 rounded-sm text-white">
                ...
                <p className="text-lg font-black">
                  {basket.reduce((acc, i) => acc + i.quantity, 0)} ta
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="flex justify-between pt-4 border-t">
            ...
            <Button
              ...
              disabled={isPending}
            >
              {isPending ? "Biriktirilmoqda..." : `O'tkazish`}
            </Button>
          </div>
        </div>
      )}
...
function Badge({
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-sm text-[9px] font-bold uppercase tracking-tight ${className}`}
    >
      {children}
    </span>
  );

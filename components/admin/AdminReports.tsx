...
import {
	Download, Package, TrendingUp, ShoppingCart, Calendar, History, Award, BarChart3, PieChartIcon
} from "lucide-react";
...
import { AnalyticInventoryData, InventorySeller } from "@/interface/analytic.type";
...
export default function AdminReports() {
	...
	const { data: reports, isLoading: reportsLoading } = useReports(filterYear, filterMonth) as { data: Report, isLoading: boolean };
	const { data: analytics, isLoading: analyticsLoading } = useAnalytics() as { data: AnalyticInventoryData, isLoading: boolean };
	...
	const barData = useMemo(() => analytics ? analytics.sellers.map((s: InventorySeller) => ({
		...
		<TabsContent value="reports" className="mt-4 space-y-8">
			...
			<div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
				<div className="space-y-1">
					...
					<p className="text-muted-foreground text-sm font-medium">Ombor va sotuvlar monitoringi</p>
				</div>
				...
				<div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
					...
					>
						{isExporting ? <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white" /> : <Download className="h-4 w-4" />}
						Excel Hisobot
					</Button>
				</div>
			</div>
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
				<Card className="lg:col-span-2 border-none shadow-sm bg-white overflow-hidden">
					<CardHeader className="flex flex-row items-center justify-between pb-2">
						<div>
							<CardTitle className="text-base font-bold text-gray-800">Sotuvchilar Balansi</CardTitle>
						...
						<ResponsiveContainer width="100%" height="100%">
							<BarChart data={barData}>
								...
								<RechartsTooltip
									...
									formatter={(val: number) => [`${val.toLocaleString()} so'm`, "Summa"]}
								/>
					...
					<CardContent className="h-[300px] flex flex-col justify-center">
						<ResponsiveContainer width="100%" height="100%">
							<PieChart>
								...
								<RechartsTooltip formatter={(val: number) => `${val.toLocaleString()} so'm`} />
			...
			<div className="space-y-4">
				...
				<div className="grid gap-4">
					{analytics?.sellers.map((seller: InventorySeller) => (
						<Card key={seller._id} className="border border-gray-100 shadow-sm hover:border-blue-200 transition-all rounded-2xl overflow-hidden bg-white">
									...
									<AccordionTrigger className="px-6 py-4 hover:no-underline group">
										<div className="flex justify-between items-center w-full pr-4 text-left">
											<div className="flex items-center gap-4">
												...
												<div>
													<p className="font-bold text-gray-900 leading-none mb-1">{seller.firstName} {seller.lastName}</p>
													<p className="text-xs text-muted-foreground font-medium">@{seller.username}</p>
												</div>
											</div>
											<div className="text-right hidden sm:block">
												<p className="text-[10px] font-bold uppercase tracking-wider text-gray-400">{seller.productCount} Umumiy Mahsulotlar</p>
												<p className="text-[10px] font-bold uppercase tracking-wider text-gray-400"> Balans</p>
												<p className="font-black text-blue-600 tabular-nums">{seller.totalValue.toLocaleString()}{`so'm`}</p>
											</div>
										</div>
									</AccordionTrigger>
											...
											<Table>
												<TableHeader className="bg-gray-100/80">
													<TableRow className="hover:bg-transparent border-gray-200">
														<TableHead className="font-bold text-gray-600">Mahsulot</TableHead>
														<TableHead className="text-center font-bold text-gray-600">Soni</TableHead>
														<TableHead className="text-right font-bold text-gray-600">Qiymati</TableHead>
													</TableRow>
												</TableHeader>
												<TableBody>
													...
													<TableRow className="border-gray-100">
														<TableCell colSpan={3} className="text-center py-8 text-muted-foreground italic text-xs">
															Hozircha faqat umumiy statistika mavjud. Detallashtirilgan API integratsiyasi Atabekni kutilmoqda.
														</TableCell>
													</TableRow>
												</TableBody>
											</Table>
...
function StatItem({ title, value, icon, desc, isPrice, unit }: StatItemProps) {
	const formattedValue = isPrice ? `${Number(value).toLocaleString()} so'm` : `${value} ${unit || ''}`;
	...
	return (
		<div className="mt-4 space-y-8 animate-pulse p-4">
			...
			<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
				{[1, 2, 3, 4].map((i) => <div key={i} className="h-32 bg-gray-100 rounded-2xl border" />)}
			</div>
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">

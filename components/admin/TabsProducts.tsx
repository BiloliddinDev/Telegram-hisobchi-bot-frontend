...
import { ImportCostPriceDialog } from "./ImportCostPriceDialog";
      ...
      <Tabs defaultValue="list" className="w-full">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 bg-white p-4 border rounded-sm shadow-sm">
          <TabsList className="grid grid-cols-2 w-full sm:w-[400px] h-10 bg-slate-100 p-1 rounded-sm">
            <TabsTrigger
              value="list"
              className="data-[state=active]:bg-white data-[state=active]:shadow-sm rounded-sm font-bold text-[11px] uppercase tracking-wider gap-2"
            >
                ...
                <>
                  ...
                  <div className="flex items-center justify-between px-6 py-4 border-t bg-gray-50/30">
                    <div className="text-[11px] font-bold text-gray-400 uppercase tracking-tighter">
                      Sahifa {page} / {totalPages} — Jami{" "}
                      {productData?.totalDocs || 0} mahsulot
                    </div>
                    <div className="flex gap-2">
                      <Button
                        variant="outline"
          ...
          <Card className="rounded-sm border-gray-200 shadow-none">
            <CardHeader className="border-b bg-gray-50/50">
              <div className="flex justify-between items-center">
                <div>
                  <CardTitle className="text-sm font-black uppercase tracking-widest text-gray-800">
                    Kategoriyalar jadvallari
                  </CardTitle>
  ...
  return (
    <div className="w-full p-4 space-y-4">
      ...
      {[...Array(rows)].map((_, i) => (
        <div
          key={i}
          className="flex gap-4 items-center border-b pb-4 last:border-0 pt-2"
        >
          {[...Array(cols)].map((_, j) => (
            <Skeleton
              key={j}
...
function Badge({
  ...
  return (
    <span
      className={`inline-flex items-center px-2.5 py-1 rounded-sm text-[10px] font-bold uppercase tracking-wider ${className}`}
    >
      {children}
    </span>
  );

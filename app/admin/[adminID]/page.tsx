...
export default function AdminSellerDetailPage() {
  ...
  const [selectedStock, setSelectedStock] = useState<SellerStock | null>(null);
  const [tempQuantity, setTempQuantity] = useState<number>(0);


  const openEditDrawer = (stock: SellerStock) => {
      ...
      <div className="flex flex-col gap-6 p-6 bg-background min-h-screen">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex gap-2">
            ...
            >
              <Phone size={12} className="mr-2 text-primary" /> {seller.phoneNumber || "Noma'lum"}
            </Badge>
            <Badge
                variant="outline"
                className={`px-3 py-1.5 border-slate-200 shadow-sm font-bold text-[10px] uppercase ${
                    seller.isActive ? "bg-green-50 text-green-600" : "bg-red-50 text-red-600"
                }`}
            >
          ...
          <div className="bg-white p-2 border rounded-sm shadow-sm mb-6">
            <TabsList className="grid w-full grid-cols-2 sm:w-[400px] h-10 bg-slate-100 p-1 rounded-sm">
              ...
              <TabsTrigger
                  ...
                  className="data-[state=active]:bg-white data-[state=active]:shadow-sm font-bold text-[11px] uppercase tracking-wider gap-2 rounded-sm"
              >
                <ArrowLeftRight size={14} className="text-primary" /> Zaxirani qaytarish
              </TabsTrigger>
            </TabsList>
          </div>
              ...
              <CardContent className="p-0">
                <div className="overflow-x-auto">
                  <Table>
                    ...
                    <TableBody>
                      {stocksLoading ? (
                          <TableRow><TableCell colSpan={4} className="h-40 text-center"><Skeleton className="h-10 w-full" /></TableCell></TableRow>
                      ) : filteredStocks.length > 0 ? (
                          filteredStocks.map((stock) => (
                              <TableRow onClick={() => openEditDrawer(stock?.product?._id)} key={stock._id} className="hover:bg-slate-50/50">
                                ...
                                <TableCell className="px-6 py-4 text-center">
                                  <span className="text-xs font-black text-primary bg-primary/10 px-2 py-1 rounded-sm">{stock?.quantity} ta</span>
                                </TableCell>
                                <TableCell className="px-6 py-4 text-right">
                                  <p className="text-xs font-black text-slate-900">{stock?.product?.price.toLocaleString()} UZS</p>
                                </TableCell>
                              </TableRow>
                          ))
                      ) : (
          ...
          <TabsContent value="returns" className="mt-0 outline-none">
            <Card className="rounded-sm border-slate-200 shadow-none border-dashed bg-slate-50/50">
              <CardContent className="h-64 flex flex-col items-center justify-center text-center space-y-4">
                ...
                <div>
                  <h3 className="text-sm font-black uppercase text-slate-800 tracking-widest">Zaxirani qaytarish</h3>
          ...
          <DrawerContent className="bg-white border-none rounded-t-[40px]">
            ...
            <div className="mx-auto w-full max-w-sm px-6">
              <DrawerHeader className="pt-8">
                <DrawerTitle className="text-center text-xs font-black uppercase tracking-widest text-slate-400">Zaxira miqdori</DrawerTitle>
              ...
              <DrawerFooter className="pb-12 pt-0">
                ...
                >
                  {isSaving ? <Loader2 className="animate-spin mr-2" /> : "Tasdiqlash"}
                </Button>
                <DrawerClose asChild>
                  <Button variant="ghost" className="h-12 font-bold text-slate-400 uppercase text-[10px]">Bekor qilish</Button>
                </DrawerClose>
              </DrawerFooter>
            </div>
          </DrawerContent>
